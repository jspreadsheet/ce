...
<html>
  ...
  <p>
    ...
    <input type="button" value="updateSelectionFromCoords(2,2,3,3)" id="btn2" />
  </p>
  ...
  <script>
    ...
    document.getElementById("btn2").onclick = () =>
      table[0].updateSelectionFromCoords(2, 2, 3, 3);
  </script>
</html>
```
...
export default function App() {
  ...
  return (
    <>
      ...
      <button onClick={() => spreadsheet.current[0].selectAll()}>
        Select all
      </button>
      <button
        onClick={() =>
          spreadsheet.current[0].updateSelectionFromCoords(2, 2, 3, 3)
        }
      >
        Select coords (2,2,3,3)
      </button>
    </>
  );
}
...
<template>
  ...
  <input
    type="button"
    value="Select coords (2,2,3,3)"
    @click="updateSelectionFromCoords(2, 2, 3, 3)"
  />
</template>
