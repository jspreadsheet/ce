...
(function ($) {
  ...
  var methods = {
    ...
    create: function (obj, options) {
      ...
      if (options.readonly == 1) {
        ...
        $(obj).on('keydown', function (e) {
          ...
          if (code == 13 || code == 32 || code == 8 || code == 37 || code == 39) {} else {
    ...
    fromFormatted: function (date, format) {
      ...
      // TODO: format bug - always getting MM/DD/YYYY

      // excel hack.  date is d/m/yy and need to flip it to yyyy-mm-dd format
      var sub = date.split('/');
      if (sub.length == 3) {
        m = parseInt(sub[0]);
        d = parseInt(sub[1]);
        y = (parseInt(sub[2]) < 100) ? "20" + parseInt(sub[2]) : parseInt(sub[2]);
        h = "00";
        i = "00";
        s = "00";
        test = 2;
